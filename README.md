...
[下载 Windows 安装包](https://github.com/ww085213/Mineradio-LX-Music/releases/download/v1.5.6.3/Mineradio.Setup.1.5.6.3.exe) / [查看 v1.5.6.3 Release](https://github.com/ww085213/Mineradio-LX-Music/releases/tag/v1.5.6.3)
...
- 保留 1.5.6.2 的覆盖升级白名单迁移与分级自修复，原版、旧版和其他二创版本可继续直接覆盖安装。
- 视频壁纸加载或硬件解码失败时只对本次会话降级，不再清空用户保存的 `backgroundMedia`；主页优先显示，视频随后初始化。
- 桌面歌词锁定后保持稳定点击穿透，修复主窗口靠近歌词层时闪烁；解锁后仍可拖动，“中键解锁”提示不再常驻。
- 电台扩展至 29 个主题，过滤播客、朗读和 ASMR 等非歌曲内容；支持收藏电台并置顶。
- DIY 面板新增功能搜索和折叠分组，修复中文输入与搜索定位；主、简洁播放条的音量按钮及弹层均支持滚轮调节。
- 加入最小化和隐藏后的渲染降频、缓存释放与应用内存回收，降低后台资源占用。

[查看 Mineradio 1.5.6.3 完整更新说明](RELEASE_NOTES_1.5.6.3.md)

SHA-256：`dee0b02106f0dfad3d0c6210e54909682dc0d88d9ad6737faa08b807de8b18ed`
...
- 音乐电台：支持 29 种私人漫游、场景、风格和能量模式，自动生成纯歌曲队列，并可收藏置顶常用电台。
