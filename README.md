...
- 修复大歌单只显示 64 首的问题。
- 大歌单现在可以继续加载，不会卡在 64 首。
- 主页歌单支持拖动调整顺序。
- 左侧“本地库”会同步主页歌单顺序。
- 自定义歌单、文件夹歌单和 LX 歌单会按用户整理后的顺序显示。
- 修复左侧歌单在部分筛选状态下可能被清空的问题。
...
- 以下情况更容易匹配到歌词和封面：
  ...
  - 本地音频标签里有标题或歌手
  - 在线源能搜到对应歌曲
  - 同目录存在同名 `.lrc` 或 `.txt` 歌词文件
- 以下情况仍可能找不到歌词或封面：
  - 歌名太乱或缺少歌手信息
  - 翻唱、remix、剪辑版、DJ 版等版本不明确
  - 纯音乐、小众歌曲或在线源没有收录
  - 本地音频标签缺失或错误

### 歌词系统

- 新增局内歌词 1 / 2 / 3 / 4 / 6 / 8 行显示。
- 新增局内歌词竖排显示。
- 竖排切换时不会残留横排歌词。
- 横排恢复当前句加下一句等待上来的显示效果。
- 桌面歌词大小、透明度、高度、帧率、布局和对齐设置已移到“歌词”设置里。
- 桌面歌词三行、四行现在真正生效。
...
- 修复开启动态壁纸时，“星河 / 星球”等视觉预设消失的问题。
- 动态壁纸下粒子律动已调回可见。
- 保留壁纸、封面背景、3D 歌单架和播放态视觉参数。

### 播放显示

- Windows 媒体控制会尽量显示 Mineradio 正在播放的歌名和歌手。
- 调整落雪双端同步自动接管逻辑，减少媒体信息显示成 LX Music 的情况。

### 音源导入优化

- 修复部分兼容音源导入失败的问题。
- 放宽音源脚本文本预检，支持更多混淆写法和 `globalThis['lx']` / 十六进制属性访问形式。
- 兼容第三方音源注册的可选 LX 事件，避免非必要事件导致导入失败。
- 保留真实沙箱初始化校验，坏文件仍会被拦截。
- 已验证 2026 年 5 月 4 日音源包内 14 个 `.js` 脚本可正常导入。

### 安全和稳定

- 新增主窗口外部跳转保护。
- 修复 LX 音源脚本触发 `EPIPE broken pipe` 后导致主进程弹 JavaScript 错误的问题。
- 已做前端内联脚本和 Node / Electron 主文件语法检查。
