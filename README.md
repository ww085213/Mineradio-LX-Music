...
MR 不包含、不分发 LX Music，不包含音乐音源，也不提供或分发任何受版权保护的音频内容。用户需要自行安装 LX Music，并在 LX Music 设置中开启开放 API 后，MR 才能读取本机播放状态。

## Mineradio 1.4.7

[下载 Windows 安装包](https://github.com/ww085213/Mineradio-LX-Music/releases/download/v1.4.7/Mineradio.Setup.1.4.7.exe) · [观看新版介绍视频](https://github.com/ww085213/Mineradio-LX-Music/releases/download/v1.4.7/Mineradio-1.4.7-intro.mp4) · [查看完整发布说明](https://github.com/ww085213/Mineradio-LX-Music/releases/tag/v1.4.7)

> Windows x64 版本，支持自选安装目录。安装包暂未使用商业代码签名，首次运行时 Windows 可能显示安全提醒。

## GitHub 项目描述

MR 是一个独立的桌面音乐可视化工具，接入用户本机 LX Music（落雪音乐）开放 API，实现读取并播放用户本机 LX Music 歌单。MR 不包含、不分发 LX Music、音乐音源或受版权保护的音频内容。

## 界面示例

> 以下截图仅用于展示 MR 的界面形态和本机联动效果。截图中的歌曲名、歌手名、专辑封面、歌单封面、壁纸图片或视频画面均来自用户本机环境或第三方软件显示内容，不代表本仓库内置、分发或授权这些内容，相关版权归各自权利人所有。

### 3D 歌单架与 Wallpaper Engine 背景

![3D 歌单架与 Wallpaper Engine 背景](docs/screenshots/playlist-wallpaper-example.png)

### 歌词舞台与播放态视觉

![歌词舞台与播放态视觉](docs/screenshots/lyric-stage-playback-example.png)

### LX Music 本机开放 API 播放状态示例

![LX Music 本机开放 API 播放状态示例](docs/screenshots/lx-music-local-api-example.png)

### Wallpaper Engine 壁纸选择

![Wallpaper Engine 壁纸选择](docs/screenshots/wallpaper-engine-browser-example.png)

### 背景媒体与 Wallpaper Engine 设置入口

![背景媒体与 Wallpaper Engine 设置入口](docs/screenshots/wallpaper-engine-settings-example.png)

## 致谢与版权边界

- 感谢原项目 [@XxHuberrr/Mineradio](https://github.com/XxHuberrr/Mineradio)。本项目是在个人学习、二次开发和本地化使用需求下整理的版本。
- 感谢 [@lyswhut/lx-music-desktop](https://github.com/lyswhut/lx-music-desktop) / LX Music。MR 仅连接用户本机自行开启的 LX Music 开放 API，不包含、不分发 LX Music 程序本体。
- MR 不提供音乐音源，不绕过任何平台权限，不随仓库分发歌曲文件、专辑封面、歌单数据、壁纸素材或受版权保护的音频内容。
- 如果你要公开发布二进制包，请确认包内不包含个人缓存、音源配置、歌曲文件、第三方壁纸文件或未经授权的素材。

## 核心定位

- 本地音乐播放器
- LX Music 本机开放 API 连接工具
- 歌词舞台与桌面歌词播放器
- 3D 歌单架和音乐可视化桌面体验
...
- 去账号化使用路径：弱化官方平台登录和平台账号体系，优先服务本地音乐与用户本机 LX Music 开放 API。
- 本地音乐优先：加强本地文件夹扫描、本地音乐库、本地歌单和本地播放队列。
- LX Music 连接：通过用户本机开启的 LX Music 开放 API 读取播放状态、歌单、封面、进度和歌词。
...
需要用户自行安装 LX Music，并在 LX Music 设置中开启开放 API。MR 只连接用户本机开放的 API，用于读取播放状态、歌单、封面、进度和歌词等信息。

MR 不会内置 LX Music，不会打包 LX Music，不会提供音乐音源，也不会随仓库发布歌曲文件。
...
如果需要在线播放能力，请用户自行安装并配置 LX Music。MR 只读取用户本机 LX Music 开放 API 暴露的播放状态和相关元数据。
