...
function fxConsoleFocusEntry(entry) {
  ...
  requestAnimationFrame(function () {
    ...
    var panel = document.getElementById('fx-panel');
    if (typeof scrollFxFeatureIntoPanel === 'function') {
      scrollFxFeatureIntoPanel(panel, entry.element, reduceMotion ? 'auto' : 'smooth');
    } else if (panel) {
      var panelRect = panel.getBoundingClientRect();
      var entryRect = entry.element.getBoundingClientRect();
      var targetTop = panel.scrollTop + entryRect.top - panelRect.top - Math.max(0, (panel.clientHeight - entryRect.height) / 2);
      panel.scrollTop = Math.max(0, Math.min(targetTop, Math.max(0, panel.scrollHeight - panel.clientHeight)));
    }
