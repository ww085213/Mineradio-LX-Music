...
(function (global) {
  ...
  // The current Mineradio build already uses preset 7 for its native terrain.
  // Keep the original Ajin preset intact, but mount it in the first free slot.
  var INDEX = 13;
  var RIPPLE_MAX = 10;
  ...
  function sonicCustomGroundTheme(fx) {
    ...
    if (!(fx && fx.sonicAdaptiveSongColor === true)) return custom;
    var cover = sonicCoverGroundTheme(fx);
    custom.base1.lerp(cover.base1, 0.24);
    custom.base2.lerp(cover.base2, 0.28);
    custom.coolCore.lerp(cover.coolCore, 0.38);
    custom.warmCore.lerp(cover.warmCore, 0.38);
    custom.ripple.lerp(cover.ripple, 0.42);
    return custom;
  }
